...
class JBGLogger(terminal.Logger):
    ...
    def get_log_output_html(self) -> str:
        """HTML log suitable for rich emails (tables look good)."""
    ...
    def print_always(self, *args, html_function=None, **kwargs) -> None:
        """Prints rich output in notebook; logs plain text to buffer (always)."""
        raw = " ".join(str(a) for a in args)

        # Rich display for notebook:
    ...
    def show_statistics_on_dataset(self, dataset):
        ...
        try:
            ...
            correlations = dataset.corr('pearson', numeric_only=True)
        ...
        try:
            ...
            skew = dataset.skew(numeric_only=True)
    ...
    def display_matrix(self, title: str, matrix: pd.DataFrame, print_always: bool = True, precision: int = 2) -> None:
        """Shows rich table in notebook, but logs a text + HTML snapshot to internal buffers (no duplicate text in output)."""
        if self._enable_quiet and not print_always:
            return self

        h3 = partial(html_wrapper, "h3")
        # Keep the formatted heading in the notebook
        ...
        # ---------- Logging snapshots (do NOT send to TeeStream / stdout) ----------
        # Text snapshot (good for plain-text emails)
        try:
            # nicer layout if user has 'tabulate' installed
            text_snapshot = matrix.to_markdown(index=True)  # pandas>=1.0
        except Exception:
            text_snapshot = matrix.to_string(index=True)

        # HTML snapshot (great for HTML emails)
        html_snapshot = matrix.to_html(index=True, border=1)
...
def main():
    ...
    myLogger = JBGLogger(quiet=False, in_terminal = True)
    message = "Hello terminal and buffer!"

    myLogger.print_info(message)
    
     # Test dataframe display
    df = make_dummy_dataframe()
    myLogger.display_matrix("Dummy Test DataFrame", df)
    
    streamed_log = myLogger.get_log_output()
    print("And the streamed log...\n" + str(streamed_log))
